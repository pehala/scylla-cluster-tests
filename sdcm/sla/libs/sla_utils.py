...
class SlaUtils:
    STRESS_READ_CMD = (
        "cassandra-stress read cl=QUORUM duration={duration} -mode cql3 native user={user} "
    ...
    def get_c_s_stats(self, read_queue, users, statistic_name, tester):
        ...
        for i, read in enumerate(read_queue):
            stat_rate, username = self.get_statistic_value_from_cassandra_stress_result(
                read_run=read, user_name=role_names[i], statistic_name=statistic_name, tester=tester
            )
    ...
    def validate_io_queue_operations(
        self,
        start_time,
        end_time,
        read_users,
        ...
        for node in db_cluster.nodes:
            ...
            for step in ["30s", "45s", "60s", "120s"]:
                LOGGER.debug(
                    "Query 'scylla_io_queue_total_operations' on the node %s with irate step %s ", node_ip, step
                )
            ...
            if not io_queue_total_operations:
                ...
                WorkloadPrioritisationEvent.EmptyPrometheusData(
                    message=f"Failed to get io_queue_total_operations data from Prometheus for node {node_ip}",
            ...
            if not expected_ratio:
                ...
                if load_high_enough is None:
                    node_cpu = prometheus_stats.get_scylla_reactor_utilization(
                        start_time=start_time, end_time=end_time, instance=node_ip
                    )
                result.append(
                    self.validate_io_queue_operations_relatively_to_share(
                        roles_full_info=roles_full_info,
                        node_ip=node_ip,
                        node_cpu=node_cpu,
    ...
    @staticmethod
    def validate_io_queue_operations_relatively_to_share(
        roles_full_info: dict,
        node_ip: str,
        load_high_enough: bool = None,
        node_cpu: float = None,
        publish_wp_error_event=False,
        possible_issue=None,
    ):
        ...
        if shares[0] == shares[1]:
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message="Both roles have same shares. Runtime can not be validated", severity=Severity.NORMAL
            ).publish()
            return None
        ...
        for service_level in roles_full_info.values():
            ops_per_sl_group.append(
                f"{service_level['sl_group']} (shares "
        ...
        if load_high_enough is None and node_cpu is None:
            WorkloadPrioritisationEvent.RatioValidationEvent(
                message=message % " Maybe due to not enough high load.", severity=Severity.WARNING
            ).publish()
        else:
            ...
            if publish_wp_error_event:
                WorkloadPrioritisationEvent.RatioValidationEvent(
                    message=error_message, severity=Severity.ERROR
                ).publish()
    ...
    @retrying(
        n=40,
        sleep_time=3,
        message="Wait for service level has been propagated to all nodes",
        allowed_exceptions=(
            Exception,
            SchedulerGroupNotFound,
            WrongServiceLevelShares,
        ),
    )
    def wait_for_service_level_propagated(self, cluster, service_level):
        ...
        for node in cluster.nodes:
            LOGGER.debug(
                "Start wait for service level propagated for service_level %s on the node '%s'",
                service_level.name,
                node.name,
            )
            if not (
                scheduler_shares := self.get_scheduler_shares_per_group(
                    node=node, scheduler_group_name=service_level.scheduler_group_name
                )
            ):
                raise SchedulerGroupNotFound(
                    f"Scheduler groups for {service_level.name} service levels is not created on the "
                    f"node '{node.name}'. Maybe the issue scylla-cluster-tests#7082"
                )

            if len(scheduler_shares) != node.scylla_shards:
                raise WrongServiceLevelShares(
                    f"Expected that scheduler group is created on every Scylla shard for {service_level.name} "
            ...
            if len(set(scheduler_shares)) > 1:
                raise WrongServiceLevelShares(
                    f"Expected same shares on every shard for {service_level.name} service level but "
            ...
            if scheduler_shares[0] != service_level.shares:
                raise WrongServiceLevelShares(
                    f"Expected {service_level.name} service level with '{service_level.shares}' shares "
                    f"but actually it is {scheduler_shares} shares on the node '{node.name}'"
                )
            LOGGER.debug(
                "Finish wait for service level propagated for service_level %s on the node '%s'",
                service_level.name,
                node.name,
            )
