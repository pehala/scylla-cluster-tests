...
def get_db_arch(config_files, backend):
    ...
    # TODO: add handlers for GCE and OCI when available
    if backend in ("aws", "k8s-eks"):
        ...
        vm_arch = azure_utils.get_arch_from_azure_instance_type(instance_type)
        click.echo("arm64" if vm_arch == VmArch.ARM else "x86_64")
        sys.exit(0)
...
def send_email(
    ...
    client = init_argus_client(argus_test_id, use_tunnel=get_argus_use_tunnel_from_env(), log_dir=logdir)
    run = client.get_run()
...
def create_test_release_jobs(branch, username, password, sct_branch, sct_repo):
    ...
    scylla_doctor_path = f"{server.base_sct_dir}/jenkins-pipelines/scylla-doctor"
    server.create_job_tree(scylla_doctor_path)
